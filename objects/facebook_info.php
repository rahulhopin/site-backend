...
public function __construct ($row)
{
     ...
     // get friends count
     $query = "select count(*) as num from friends where fbid1 = ". $this->fbid;
     $sth = $db->prepare($query);
     $sth->execute();
     $result = $sth->fetch(PDO::FETCH_ASSOC);
     $this->friends_count = 0;
     if(isset($result['num']))
       $this->friends_count = $result['num'];
...
public function getData()
{
 ...
 $fb_array = array( "fbid" => $this->fbid , "firstname" => $this->firstname,  "lastname" => $this->lastname , "username" => $this->username, "email" => $this->email ,
                           "works_at" => $this->workplace,"lives_in" => $this->current_city , "hometown" => $this->hometown, "study_at" => $this->study,"image_url" =>  $this->pic, "gender" => $this->gender, "phone" => $this->phone , 'friends_count' => $this->friends_count , 'email_verified' => $this->email_verified);
