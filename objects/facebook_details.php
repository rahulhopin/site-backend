...
function getFriendsquery($id, $token){
 $fbURL = "https://graph.facebook.com/";
 return $fbURL . $id . "/friends?fields=name,id,username,installed&access_token=" . $token;
}
...
if(!isset($userid) || !isset($fbtoken)  || !isset($fbid)){
 ...
 $sql = "Insert IGNORE into friends (fbid1, fbid2, username2,name1, name2,installed) values"; 
 $first=1;
 $friends_ids = array();
 foreach($friends as $friend){
  ...
  $username2 = addslashes($friend['username']);
  $installed = 0;
  ...
  $friends_ids[] = $fbid2;
  if($first==1){
   $sql .= " ($fbid, $fbid2, '$username2' , '$name', '$name2' , $installed)";
   $first=0; 
  }else{
   $sql .= ", ($fbid, $fbid2,'$username2', '$name', '$name2' , $installed)"; 
  }
