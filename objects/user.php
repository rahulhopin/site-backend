...
class User extends dbclass {
	...
	function __construct(){
		...
		$this->fields['platform'] = new Field('platform','platform',0);
...
function add($arguments){
  ...
  // for site get the required params
  if($GLOBALS['site'] == 1){
	$arguments['uuid'] = $GLOBALS[BCOOKIE];
	$arguments['platform'] = 'site';
	$arguments['password'] = md5($arguments['password']);
...
error_log("======= " . print_r($arguments , true));
  $primary = 'uuid';
   ...
   $response .= $json_msg->getMessage();
		if($GLOBALS['site']== 1){
			SiteUtils::sendConfirmation($result[0]['id'], $arguments['email']);
		} 		 
  }
  return;
}	
