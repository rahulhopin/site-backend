...
class Xmpp_Bosh
{
    ...
    {
        $hash = base64_encode( $user . "@hopin.co.in\0" . $user .
"\0" . $password ) . "\n";
        $rid = rand();
        $jid = $user . "@hopin.co.in";
        // when using openfire 
        $body = "<body rid='" . $rid . "' xmlns='http://jabber.org/protocol/httpbind' to='hopin.co.in' xml:lang='en' wait='60' hold='1' window='5' content='text/xml; charset=utf-8' ver='1.6' xmpp:version='1.0' xmlns:xmpp='urn:xmpp:xbosh'/>";
...
"</auth></body>";
        ...
        $rid ++;
        $body = "<body rid='" . $rid . "' xmlns='http://jabber.org/ 
protocol/httpbind' sid='" . $sid . "' to='xxxxxx.yyy' xml:lang='en' 
...
xmlns='jabber:client'><bind xmlns='urn:ietf:params:xml:ns:xmpp-bind'/></iq></body>";
...
xmlns='urn:ietf:params:xml:ns:xmpp-session'/></iq></body>";
  ...
  private function __sendBody ( $body )
    {
        //when using openfire 
        $ch = curl_init( "http://hopin.co.in/http-bind/" );
