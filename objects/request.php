...
function showMatchesOld($matches,$fbid=0){ 
 ...
 function deleteRandom($arguments){
  ...
  //insert into history table always
		parent::insert('request_history',$this->fields);
		$result = parent::select('request',array('*'),array('user_id' => $arguments['user_id']));
		$json_msg = new JSONMessage();
 ...
 function addCarpoolRequest($arguments){
  ...
  if(isset($result[0]['id'])){
   $sql = "UPDATE carpool SET src_latitude=$src_lat, src_longitude=$src_lon, dst_latitude=$dst_lat, dst_longitude=$dst_lon, src_address=\"$src_add\", dst_address=\"$dst_add\", time=\"$ttime\", src_locality=\"$src_locality\", dst_locality=\"$dst_locality\", women=$women, facebook=$facebook , type=$type WHERE user_id=$user_id";
  }else{
   $sql = "INSERT INTO carpool (user_id, src_latitude, src_longitude, dst_latitude, dst_longitude, src_address, dst_address, time, src_locality, dst_locality, women, facebook ,type) VALUES ($user_id, $src_lat, $src_lon, $dst_lat, $dst_lon, \"$src_add\", \"$dst_add\", \"$ttime\", \"$src_locality\", \"$dst_locality\", $women, $facebook ,$type)";
  }
  parent::execute($sql);
  $sql = "INSERT INTO request_history (user_id, src_latitude, src_longitude, dst_latitude, dst_longitude, src_address, dst_address, time, src_locality, dst_locality, women, facebook,category) VALUES ($user_id, $src_lat, $src_lon, $dst_lat, $dst_lon, \"$src_add\", \"$dst_add\", \"$ttime\", \"$src_locality\", \"$dst_locality\", $women, $facebook,1)";
  parent::execute($sql);
...
function matchCarpoolRequest($user_id,$lat_src,$lon_src,$lat_dst,$lon_dst, $type, $ttime, $women, $facebook, $users = array()){
 ...
 function getCarpoolMatches($arguments){
  ...
  if($user_id != 0 && $site!=1) $this->sendGCM($matches,$user_id,0);
  return $this->showMatches($matches,$fbid,1);
}
...
function getFriendsToInvite($fbid, $offset=0,$limit=15){
  $friends = parent::execute("select fbid2,name2,username2,installed from friends where fbid1=$fbid and invited=0 limit $offset,$limit  ");
  $res = array();
  while ($row = $friends->fetch_assoc()) {
   $res[] = array('fbid'=> $row['fbid2'], 'name'=>$row['name2'], 'username' => $row['username2'] , 'installed' => $row['installed']);
  }
 return $res;
}
function showMatches($matches,$fbid=0,$carpool = 1){
 ...
 if(count($match_ids)>0){
  ...
  if(count($resp) <= 1){
   $friendsToinvite =  $this->getFriendsToInvite($fbid);    
 }
*/
  $json_msg = new JSONMessage();
  //$json_msg->setBody (array("NearbyUsers" => $resp, "FriendsToInvite" =>$friendsToinvite)); 
  $json_msg->setBody (array("NearbyUsers" => $resp)); 
  $response .= $json_msg->getMessage(); 
		return $json_msg->getMessage();
